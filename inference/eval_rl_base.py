...
import numpy as np
import pandas as pd
from utils.base_template import generate_prompt_for_inference, generate_prompt_reasoning_for_inference
...
def prepare_test_dataset_and_dataloader(args, tokenizer):
    ...
    with accelerator.main_process_first():
        ...
        if args.get('ucot', True):
            print("************************Using UCOT prompts for inference***********************")
            prompts_test = df_test.apply(generate_prompt_reasoning_for_inference, axis=1, result_type="expand")
            
        elif 'generate_prompt_for_inference' in globals():
            prompts_test = df_test.apply(generate_prompt_for_inference, axis=1, result_type="expand")
        else:
...
def evaluate_generation(args, model, dataset, dataloader, tokenizer):
    ...
    for idx, batch in tqdm(enumerate(dataloader), total=len(dataloader), disable=not accelerator.is_main_process,
        ...
        with torch.no_grad():
            output_ = accelerator.unwrap_model(model).generate(
                ...
                max_new_tokens=args.get('max_new_tokens', 20),
                min_new_tokens=5,
                output_scores=True,
    ...
    for pred_txt, item in zip(predictions, dataset):
        ...
        # Extract prediction using the same logic as training
    ...
    # Save results
    res_path = os.path.join(args["model_dir"].rstrip("/"), f"test_results_p{args['top_p']}_t{args['temperature']}_checkpoint{args['best_checkpoint']}.json")
    ...
    summary = {
        "accuracy": accuracy,
        "macro_f1": macro_f1,
        "claassification_report": classification_report(true_labels, pred_labels, target_names=['Class 0', 'Class 1'], output_dict=True),
        "total_samples": len(results),
        "valid_predictions": len(true_labels),
        "invalid_predictions": len(results) - len(true_labels)
    }

    summary_path = os.path.join(args["model_dir"].rstrip("/"), f"test_summary_p{args['top_p']}_t{args['temperature']}_checkpoint{args['best_checkpoint']}.json")
...
def load_model_and_tokenizer(args):
    ...
    best_checkpoint_path = os.path.join(args["model_dir"], args['best_checkpoint'])
...
if __name__ == '__main__':
    ...
    class TestArguments:
        ...
        ucot: bool = field(default=False)
        temperature: float = field(default=0.6)
        top_p: float = field(default=0.85)
        best_checkpoint: str = field(default="...")
