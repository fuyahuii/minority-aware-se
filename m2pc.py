# ----------------------------------------------------------------------------------
# Two cluster models (majority / minority) unsupervised learnt via EM algorithm
# EM Algorithm Loop:
#    - E-step (Expectation): Route each user subset to clusters based on perplexity scores
#      First iteration: naive split (majority → cluster 0, minority → cluster 1)
#      Subsequent iterations: assign users to cluster with lower perplexity
   
#    - M-step (Maximization):
#      Train separate LoRA adapters for each cluster
#      Continue training from previous iteration's cluster models
# ----------------------------------------------------------------------------------
...
from utils.CoPeR import generate_coper_prompt_training
from utils.base_template import generate_prompt_for_training, generate_prompt_reasoning_for_training
...
class ScriptArguments:
    ...
    # --- data paths ---
    majority_train_path: str = "data/data_split/training_majority.csv"
    minority_train_path: str = "data/data_split/training_minority.csv"
    majority_valid_path: str = "data/data_split/valid_majority.csv"
    minority_valid_path: str = "data/data_split/valid_minority.csv"

    majority_users: int = 50
    minority_users: int = 20
    train_subset: int = 6100   
    eval_subset: int = 800   
    ...
    seed: int = 1103
    max_seq_length: int = 1044 #1280 if coper else 1044
    label_max_len: int= 20 #256 if coper else 20
    em_iterations: int = 10

    use_llama_template: bool = False
    log_filename: Optional[str] = None
    template: str = "ucot"  # "coper" or "base" or "ucot"
...
class RefTrainer(Trainer):
    ...
    def evaluate_generation(self, a, model, dataset: Dataset, *,
        ...
        for ex in dataset:
            ...
            groud=generate_coper_prompt_training(ex)["output"] if a.template=="coper" else generate_prompt_reasoning_for_training(ex)["output"] if a.template=="ucot" else generate_prompt_for_training(ex)["output"]
...
def main(a: ScriptArguments):
    ...
    # ===== Final cluster mapping based on BEST checkpoints =====================
    ...
    map_path = os.path.join(a.output_dir, "cluster_map.json")
